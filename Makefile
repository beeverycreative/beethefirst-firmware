...
# BTF
btf: CFLAGS += -DBTF
btf: CFLAGS += -DFW_V='"BEEVC-$(FW_VERSION)"' -DCFG_UID=$(CFG_UID)
...
copyBinBTF_PLUS:
	cp $(OUTDIR)/$(TARGET).bin $(BINDIR)/BVC-BEETHEFIRST_PLUS-Firmware-$(FW_VERSION).BIN
...
copyBinBTF_SCHOOL:
	cp $(OUTDIR)/$(TARGET).bin $(BINDIR)/BVC-BEE_IN_SCHOOL-Firmware-$(FW_VERSION).BIN
